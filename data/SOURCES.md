...
- Sysadmin and network slices: original paraphrases that cite local `man(1)` paths (`ip`, `ss`, `resolvectl`, `NetworkManager`, `systemd.*`). Live nftables listing uses privileged `nft_list_ruleset`. The agent is not trained to emit unrestricted `sh -c` for those tools.
