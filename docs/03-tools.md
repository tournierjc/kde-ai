...
| Tool | Notes |
| --- | --- |
| `system_info` | compact `summary`, GPU, monitors (EDID brand/model), CPU, RAM, motherboard, hostname, uptime (`/proc/uptime`), `distro`, `kernel` version, `kernel_cmdline` from `/proc/cmdline` plus Limine/GRUB config, Plasma, Qt, session |
...
| `run_privileged_cmd` | `id`, `systemctl_status_unit`, `journalctl_system_n`, `dmesg` |
